...
Answer explanation: **An incorrect interface has been configured on C2 as the tunnel source. It should have been _GigabitEthernet0/0/1_.**
...
----------
**A frame is sent from a device on a local area network to the port to which it is connected. The network device has a record of the destination in its media access control (MAC) address table. The frame is malformed, and the data is corrupt.** 
**How does the network device know that the frame is malformed(so badly formed or out of shape as to be ugly)?**
...
Answer explanation: A **distribution system port is used to pass all access point normal and management traffic over an 802.1q trunk to a switch.**

----------
When using **hot standby router protocol (HSRP), what mechanism does host traffic use as a default router** for traffic?

**The configured virtual IP address**

Answer explanation: The configured virtual IP address is created in the same range as the routers in the HSRP group. The virtual IP acts as a gateway for both routers. The active router is the one that receives and responds to host traffic.

----------
**Which type of port has the greatest short mode cost when calculating the cost of a path towards a root bridge?**

**Fast Ethernet**

Answer explanation: **Fast Ethernet has a cost of 19 and would be the most costly when determining a path to the root bridge.**
...
**Note: We should not use `switchport mode trunk` in this scenario. The command switchport mode trunk would place the port in trunk mode and not limit the VLANs that pass over it.**

----------
What is the result of **running the following sequence of commands on a router running OSPF as its routing protocol? Commands:**
```
config t
Interface gig 0/0/0
Ip address 192.168.10.15 255.255.255.0
...
Answer explanation: **Designating a link as a point-to-point link in OSPF indicates that it will not send OSPF broadcast frames and not elect a DR and BDR for that subnet.** This saves on OSPF convergence time due to the lack of need for a DR/BDR election.
...
Answer explanation: **4-way handshake is the final step of the WPA3 SAE process, which will generate keys to ensure encryption**. Regular data can be transmitted after this.

----------
In traditional networking, **every router makes its own decisions about forwarding packets**, and **each switch makes its own decision about forwarding frames**. **Software Defined Archictecture uses which of the following to make this process more efficient?**
...
**Note: Syntax: ip nat inside static <Private IP> <Public IP>**

----------
What command **should an engineer use to create a route on a layer 3 device that points toward the Host IP of a storage server?**

**ip route 172.16.2.4 255.255.255.255 172.16.2.1**

Answer explanation: **This creates a host route that sends traffic for the host 172.16.2.4 via the next hop of 172.16.2.1.**

----------
A network engineer is **troubleshooting poor wireless performance in an area of the corporate building**. Based on the size of the area and number of clients, there should be enough access points to handle the traffic. **All access points have been configured on the same channel**. What should the engineer do to resolve the performance issues in this network?

**Configure the access points to transmit on nonoverlapping channels**

Answer explanation: Using **nonoverlapping channels will reduce radio frequency (RF) interference,** reducing the number of dropped frames and will allow them to respond to traffic only directed to that access point, improving performance.
...
----------
A **security camera has been installed on the roof of a corporate building with a network cable provided but no convenient way to run electricity for the camera.** What networking component can be used to resolve this issue?

**Power over Ethernet (PoE)**

Answer explanation: Power over Ethernet (PoE) sends power over ethernet cables and is able to provide both power and data to a device, such as a network camera or VoIP phone, through the network jack.

----------
What command is **used to ensure that all network devices that are pointed to the current device for time updates receive those updates based on its clock settings?**

**ntp master 3**

Answer explanation: This command would set the current network device to be a master server for network time protocol (NTP) at a stratum of 3. Stratum is the level in hierarchy from an authoritative server that the current server is excahnging time information with. **A stratum of 0 indicates the highest or most reliable server and 15 indicates the lowest or least reliable.**

----------
Which of the following **commands will provide the OSPF state, router ID, and device address for other routers in the same network?**

**show ip ospf neighbor**

Answer explanation: This command will provide all the information requested: the neighbor state, process ID, router ID, which is stated as neighbor ID, and the device’s address in the OSPF network.

----------
While configuring a **legacy switching environment, an engineer has discovered all switches configured are running Cisco’s proprietary Inter-Switch Link (ISL) protocol. The router being deployed at the edge of the environment does not support ISL.** **What command can she use on the switching environments' trunk ports to ensure compatibility with modern devices?**

**switchport encapsulation dot1q**

Answer explanation: This will configure the **trunk to use the industry standard of 802.1q encapsulation.**

----------
A network engineer is attempting to **troubleshoot a failed site-to-site virtual private network (VPN) via virtual teletype (VTY) and needs to see messages that give diagnostic information about the VPN tunnel.** Which of the following actions should the engineer take to receive the messages?

**Turn on debugging for the VPN protocol and enable logging messages to be displayed to all VTY lines**

Answer explanation: **Debugging messages are generated and sent to the console by default once the debugging for a particular process is turned on.** Debugging can be processor intensive and cause issues with device performance, and therefore, should be used with care and turned off after needed. **To display the output of debugging, the engineer needs to use the terminal monitor command.**

----------
Which **option on a wireless LAN Controller (WLC) graphical user interface (GUI) provides the location to configure a quality of service (QoS) for a service set identifier (SSID)?**

**WLANs>WLANs>create new**

Answer explanation: This is the location where a wireless local area network and SSID are created and once created the WLAN can be edited to include the management of QoS configurations.

----------
What is the outcome of implementing the following cutsheet on a router? Cutsheet:
```	
config t
interface lo2
shutdown
...
exit
no ip route 192.168.12.6 255.255.255.255 192.168.10.16
ip route 192.168.12.6 255.255.255.255 192.168.12.16 # Here is the original host route next hop address replaced with 192.168.12.16
interface lo2
no shut
end
```
...
Answer explanation: "**Blocking**" is the **spanning tree port state that prevents all traffic from passing in a spanning tree broadcast domain.** **A port is placed in to blocking if it is not a root port, or designated port on a switch.**
